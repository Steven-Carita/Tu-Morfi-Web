(function () {
  ...
  function calcEnvio(tipo, localidad, cp) {
    ...
    if (cpStr.startsWith && (cpStr.startsWith("16") || cpStr.startsWith("17") || cpStr.startsWith("18"))) return 3500;
  ...
  function ensureListaResumen() {
    const panel = document.querySelector(".resumen-compra") || document.querySelector("aside") || document.body;
    ...
    let lista = wrapper.querySelector("#lista-resumen") || panel.querySelector("#lista-resumen");
    if (!lista) {
  ...
  function renderResumen() {
    ...
    carrito.forEach(p => {
      ...
      li.style.cssText = "display:grid;grid-template-columns:56px 1fr auto;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid #eee;";
    ...
    const envio = calcEnvio(tipo, locEl && locEl.value, cpEl && cpEl.value);
    ...
    if (badge) { badge.hidden = !(tipo === "envio" && envio === 0); if (!badge.hidden) badge.textContent = "GRATIS"; }
    if (msg) {
      if (tipo === "envio" && envio === 0) msg.textContent = "Envío gratis a Mataderos.";
      else if (tipo === "envio" && locEl && ["parque avellaneda", "lugano"].includes(String(locEl.value || "").toLowerCase())) msg.textContent = "Costo de envío fijo: $3.000.";
      else if (tipo === "envio") msg.textContent = "Costo estimado según zona.";
  ...
  (function wire() {
    ...
    function updateEnvio() {
      ...
      if (bloqueEnvio) bloqueEnvio.style.display = (v === "envio") ? "block" : "none";
      renderResumen();
    }
    if (selectEntrega) selectEntrega.addEventListener("change", updateEnvio);
    radiosEntrega.forEach(r => r.addEventListener("change", updateEnvio));
    ...
    if (form) {
      form.addEventListener("submit", (e) => {
        ...
        if (!datos.entrega) marcarError("#tipo-entrega", "Tipo de entrega");
        ...
        const carrito = getCarrito();
        const sub = carrito.reduce((a, p) => a + (Number(p.precio) || 0) * (Number(p.cantidad) || 1), 0);
        const env = calcEnvio(datos.entrega, datos.localidad, datos.cp);
        ...
        if (datos.entrega === "retiro") {
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              // Guardar datos del pedido como antes
              const carrito = getCarrito();
              const sub = carrito.reduce((a, p) => a + (Number(p.precio) || 0) * (Number(p.cantidad) || 1), 0);
